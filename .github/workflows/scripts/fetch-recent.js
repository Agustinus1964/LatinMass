...
const CONFIG = {
  ...
  SPREADSHEET_ID: '1vo0e6EqwdTUU0hHRCYA3RA_fhQv08Ky-RLf9PYbLdJM',
  SHEET_GID: '1666416706'
};

// Fetch channels from Google Sheets
async function fetchChannelsFromSheet() {
  const url = `https://docs.google.com/spreadsheets/d/e/2PACX-1vT${CONFIG.SPREADSHEET_ID.substring(1)}/pub?gid=${CONFIG.SHEET_GID}&single=true&output=csv`;
  
  // Try alternative URL format
  const altUrl = `https://docs.google.com/spreadsheets/d/${CONFIG.SPREADSHEET_ID}/export?format=csv&gid=${CONFIG.SHEET_GID}`;
  
  console.log('Fetching channels from Google Sheets...');
  
  let response = await fetch(altUrl);
  if (!response.ok) {
